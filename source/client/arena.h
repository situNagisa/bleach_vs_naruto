...
struct arena
{
	...
	auto render(::bvn::graphics::global_dynamic_forward_env_renderer global) -> ::bvn::gameplay::task
	{
		...
		//+ create grid vertex buffer
		{
			...
			if (::vkMapMemory(global.device(), vertex_memory.handle, 0, buffer_info.size, 0, &mapped) != ::VK_SUCCESS)
			{
		...
		try
		{
			while (!stop.stop_requested())
			{
				...
				{
					throw ::std::runtime_error{"failed to begin arena secondary command buffer"};
				}
				...
				{
					throw ::std::runtime_error{"failed to end arena secondary command buffer"};
				}
...
static_assert(::bvn::graphics::renderable<arena&, ::bvn::graphics::global_dynamic_forward_env_renderer>);
}
