...
{
	inline void render_canvas_impl(::entt::handle handle, ::glm::mat3x3 const& parent_matrix) noexcept
	{
		...
		if (handle.any_of<component::timeline_system>())
		{
			...
			for (auto&& frames : handle.get<component::timeline_system>().frames() | drop(current_frame) | take(1))
			{
				for (auto&& ff : frames)
				{
					...
					{
						controller::render_canvas_impl(h, matrix);
					}
				}
			}
		}
	...
	inline void render_canvas(::entt::handle handle) noexcept
	{
		controller::render_canvas_impl(handle, ::glm::mat3x3(1.0f));
	}
}
