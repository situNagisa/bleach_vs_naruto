...
> 2026-06-15 · 配合 `game-design.md` / `architecture.md` / `engine-spec.md`
...
| 协程 | 英雄 = 自由函数协程 + `hero_context` 驱动（见下）；**状态二分**（耐久→ECS，瞬态→协程局部）；需要组织状态时用轻量 `struct`，不额外拆辅助函数。 |
...
- **有生命周期的资源用 RAII 拥有者收尾，不在调用点逐个判空**：用 move-only 的 RAII 拥有者（构造即获取、析构即释放）管理有生命周期的对象（如 GPU 句柄），于是不必在每个调用点写 `if (handle != VK_NULL_HANDLE) destroy`——判空只写进 wrapper 的析构一次。清理期判空本身不是逻辑错误（对象可能创建到一半就抛了），但用 RAII 就根本不必在调用点写它。瞬态 GPU 资源应作为协程帧内的局部量持有（见 `render-runtime.md` 的 render task 形态）。
...
> 所有权层面的决策（"优先移交所有权、不做版本对比重建"）属架构决策，见 `architecture.md` 的「横切工程原则」。
...
hero_task kenpachi(hero_context ctx)
{
    ...
    ctx.render_scope.spawn(::stdexec::starts_on(ctx.render_gate.scheduler_value, render(ctx.view, ctx.renderer)));
    while (ctx.alive())
    {
        co_await ctx.next_tick();           // 输入 / 事件都在 ctx 里
        if (ctx.input.pressed(key::j))
        {
            co_await ctx.play("slash1", 3);
            ctx.emit_hitbox(/* ... */);
            ++combo;
        }
        ctx.move(ctx.input.move_dir);       // 耐久态 → 写 ECS
    }
}
BVN_REGISTER_HERO(kenpachi, "kenpachi");
```
**状态二分**：耐久 / 可见 / 联网态 → ECS 组件（快照只抓 registry）；瞬态 / 控制流 → 协程局部变量。
