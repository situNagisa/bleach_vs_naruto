...
| noexcept | 不抛的函数广泛标注。 |
| `[[nodiscard]]` | 标准库风：忽略即 bug 时才加（普通 getter 不加）。 |
| 类型声明 | 统一 `struct` + 显式 `public:`/`private:`。 |
...
| 协程 | 英雄 = 自由函数协程 + `hero_context` 驱动（见下）；**状态二分**（耐久→ECS，瞬态→协程局部）；可用轻量 struct 组织 helper / 具名钩子。 |
...
**auto —— 不加冗余修饰符**
```cpp
auto p = get_pointer();   // 不写 auto*
auto&& r = get_ref();     // 引用一律 auto&&（不写 auto& / auto const&）
auto v = get_value();     // 值，靠 RVO
```
...
hero_task kenpachi(hero_context ctx)
{
    ...
    while (ctx.alive())
    {
        ...
        co_yield ctx.render_task(/* ... */); // 渲染任务交回引擎执行
    }
}
