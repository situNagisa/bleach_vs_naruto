...
> 日期：2026-06-15　状态：**定稿**（经"模块 / 目录 / 接口 / 数据流 / 里程碑"逐项问答确定）
> 三件套：`game-design.md`（做什么）· `architecture.md`（T1–T9 决策日志 + 热重载详解）· **本文 = 建造蓝图**。
> ⚠️ 本文在以下点上**修订并取代** `architecture.md` 的旧说法：**英雄 = 协程**（非"虚接口 + 回调对象"）、**引擎核心不含"技能 / AbilitySystem"概念**（时间线降为可选 SDK 库）。其余一致。
...
## 2. 模块（细粒度，各独立静态库；heroes 为 DLL；client/server 为可执行）

| 模块 | 职责 | 关键内容 | 三方（直接用） |
|---|---|---|---|
| `foundation` | 基建 | 类型 / 数学 / 容器 / 日志 / 句柄 / 事件 / 序列化 | glm |
| `platform` | 系统层 | 窗口 / 输入 / 计时 / fs / **动态库加载** | SDL3 |
| `rhi` | Vulkan 薄封装 | 设备 / 交换链 / 缓冲 / 纹理 / 管线 / 提交 | volk·VMA·vk-bootstrap |
| `render` | 表现 | `IRenderer`·VulkanRenderer·RenderScene·2D 精灵·相机·**渲染任务执行**·HUD | imgui·stb |
| `compute` | 计算后端 | **ComputeScheduler**（`std::execution`） | stdexec→nvexec |
| `sim` | 仿真核心 | **裸 registry 即 World**·tick/快照·**共享交互底座**（空间 / 碰撞 / effect 通道 / 属性） | EnTT |
| `gameplay` | MOBA 规则 | 比赛流程·经济·AI·**可选标准战斗约定**·plugin host·协程英雄 runtime | — |
| `net` | 网络 | `INetTransport`·`InputCommand`·快照复制 | ENet |
| `audio` | 音频 | `IAudio` | miniaudio |
| `assets` | 资源 | 加载器·热重载·精灵表·Lua（M3 起） | stb·sol2·lua |
| `sdk` | 插件公共面 | 插件契约 + **直接纳入引擎头**（最大自由）·manifest·ABI 版本·**可选时间线 / 状态机 / 编辑器辅助库** | 无 |
...
bvn/
  ...
  engine/
    foundation/{include/bvn/foundation, src}
    platform/  rhi/  render/  compute/  sim/  gameplay/  net/  audio/  assets/  tools/
        每个：include/bvn/<模块>/*.h（公共，外部 #include <bvn/sim/...>）+ src/（实现）
  sdk/include/bvn_sdk/        # 插件契约 + 纳入引擎头 + 可选辅助库（版本化）
  heroes/                     # 自包含：每英雄一个文件夹
    kenpachi/{src, data, assets}   ichigo/{...}   ...
...
- `tick` / `snapshot` 是**自由函数**：`sim::tick(reg, inputs)`、`sim::capture(reg, back)`。
- **全局态**（tick 计数 / 种子 RNG / 比赛状态）放 `reg.ctx()`（EnTT 上下文变量）。
- 实体句柄 = `entt::entity`。组件：**通用组件在 sim**（Transform/Velocity/…），**玩法组件在 gameplay**（经济 / 队伍 / 标准战斗约定的 Health 等）。
...
```cpp
// 一个英雄 = 一个协程（示意）
HeroTask kenpachi(HeroContext ctx) {           // 启动时传入 ctx
    int combo = 0;                              // 瞬时态 = 协程局部变量（挂起自动保留）
    while (ctx.alive()) {
        co_await ctx.nextTick();                // 挂起到下一 tick；resume 时 ctx 已刷新
        auto& in = ctx.input;
        if (in.pressed(Key::J)) {               // 连招 = 顺序代码，挂起点即帧推进
            co_await ctx.play("slash1", 3);     // startup
            ctx.emitHitbox(/*…*/);              // active：走通用通道 / 或标准战斗约定
            co_await ctx.wait(3);
            ++combo;
            co_await ctx.play("recover", 5);
        }
        ctx.move(in.moveDir);                   // 引擎移动原语；写 ECS（耐久态）
        co_yield ctx.renderTask(/*当前精灵/帧/朝向*/);   // 见 §4.5（闭包快照安全数据）
    }
}
BVN_REGISTER_HERO(kenpachi, "kenpachi");
```
- **耐久 / 可见 / 联网态**（HP / 位置 / 当前帧 / 护盾）**必须写 ECS 组件**——快照只抓 registry，客户端不跑 B 的协程、只从快照渲染 B。
- "IHero" 因此是**"产出英雄协程的工厂"**，不是 vtable 回调类。
- **`HeroContext` 具体装什么 = 待定**（边界 = 那三件事 + 世界访问，不含任何"技能"概念）。

### 4.5 渲染表达：英雄返回"渲染任务"
- 英雄 `co_yield` 一个**渲染任务（sender / 协程）**；引擎把所有英雄的渲染任务 + 场景渲染**组成 render sender 图**，在 render scheduler 执行出帧。
- 渲染任务在**独立线程**跑 → 必须**闭包"快照安全"的数据 + 接收插值 alpha**；自然落法 = **英雄在 capture 快照时产出任务、闭包当帧态**。
- 非英雄的东西（地形 / 塔 / 小兵 / HUD）走引擎自己的渲染。
- **后端切换点 `IRenderer`**（RenderScene / 任务图 → 一帧）：现在 `VulkanRenderer`（内部用薄 rhi）；将来 `CudaRenderer`（CUDA 算图 + CUDA-Vulkan interop 呈现，呈现永远走 Vulkan）。
...
### 4.7 计算 / 网络
- `ComputeScheduler`（`std::execution`）：重活（寻路 / 粒子 / 大量单位 SoA）作为 sender 提交；CPU 现在 → **nvexec CUDA scheduler** 以后。`IComputeBackend ≈ 换 scheduler`。
- `INetTransport`（ENet 后端）：`InputCommand{moveDir,aim,abilityBits,seq,tick}`；**host 权威 + 快照 + 移动预测 + 和解**；快照演进 全量 → delta → 兴趣区(AOI，兼作迷雾地基)；演进到专用服务器走同一无头 sim。

### 4.8 插件加载 / SDK / 可选辅助
- 开局**扫描 `heroes/` / `plugins/`**（自包含文件夹 → DLL）+ 每插件 **manifest**（id / 版本 / 依赖 / 资源）+ **ABI 整数版本校验**（不匹配拒载 + 日志）。
- **SDK 直接纳入引擎头**（插件可直接用 ECS / 内部，最大自由）。
...
- **三方库直接用**：第三方库可大大方方直接使用，不必藏进实现、不必套隔离接口——只要**别让库影响设计**（设计先行）。注：`IRenderer` / compute scheduler / `INetTransport` 这几个抽象是为**后端可换**的设计目标（Vulkan→CUDA、netcode 演进）而留，不是为隔离库。
- **sim 解耦 / 可无头**：`sim` 不依赖 `render`/`platform`/`rhi`；`apps/server` 不链它们。
...
- **两个 CUDA 入口**：计算换 scheduler、渲染换 `IRenderer`；现在只预留（SoA + 抽象），实现在 M8。
- **内存**：`std::pmr`（arena/pool）优先；句柄而非裸指针跨系统。**错误处理分层**（边界异常 / `expected`；热路径 + 插件边界错误码）。
...
| M | 目标 | 落地重点 |
|---|---|---|
| **M0 · 地基** | 能跑的循环 + 三角形 | CMake/vcpkg/Presets · foundation · platform(SDL3) · rhi(Vulkan 三角) · **定步长循环 + 双缓冲骨架** · compute scheduler(CPU) · ImGui |
| **M1 · 渲染管线** | 看到 BvN 精灵在场景里 | `IRenderer`+VulkanRenderer · RenderScene · 3D 场景 + **2D billboard + 朝向翻转** · 斜俯视相机 · 精灵动画基础 · 资源加载+热重载 · 插值 |
...
| **M6 · 联网 MVP** | 两端联机 | `INetTransport`+ENet · host 权威快照 + 移动预测 · 房间 + bot 补位 · 无头服务器雏形 |
