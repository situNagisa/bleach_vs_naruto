...
> 渲染侧「怎么被启动」的机制见 [../render/boot.md §4 启动次序与每帧](../render/boot.md#4-启动次序与每帧)（本文只讲现状，不讲机制）。

---

## 1. entity 都放在 client 下

现阶段所有 entity 直接放在 `client` 下。这些 entity **模拟不同独立开发者各自的产出**——它们彼此独立、互不知道对方存在，正是 [../render/boot.md §1 核心哲学](../render/boot.md#1-核心哲学游戏主体只提供运行环境) 里「游戏主体只提供环境、entity 自包含」那条边界的实地演练。
