...
- **并行录制（模型 B）落地**：把内部调度器换成允许并行的，`submit()` 的 `when_all` 天然并行录各自的 secondary command buffer，再由 `submit()` 按队列顺序 join（见 [./model-ab.md §3 模型 B](./model-ab.md#3-模型-bsecondary-command-buffer并行录制-控序-设计方向)）。
