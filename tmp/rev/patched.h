...
struct dynamic_when_all_sender
{
	...
	struct operation_type
	{
		...
		struct child_receiver
		{
			...
			using _env_type = decltype(::stdexec::env{
				::stdexec::prop{::stdexec::get_stop_token,
					::std::declval<::stdexec::inplace_stop_source const&>().get_token()},
				::stdexec::get_env(::std::declval<ReceiverType const&>()),
			});

			[[nodiscard]] constexpr auto get_env() const noexcept -> _env_type
			{
				return _env_type{
